...
class MyDataset(Dataset):
    ...
    def _collect_annotations(self):
        ...
        for meta in self.config["META"]:
            ...
            if "image_root" in meta:
                for item in meta_l:
                    item["_image_root"] = meta["image_root"]
    ...
    def _get_cache_dir(config_path):
        # WARNING: This uses the config_path string directly, which may contain private/local path info.
    ...
    def __getitem__(self, index):
        try:
            sample = self.get_item_func(index)
            if hasattr(self, "additional_transform") and self.additional_transform is not None:
                sample = self.additional_transform(sample)
            return sample
    ...
    def column_names(self):
        # Returns column names for downstream code compatibility
        return ["image", "text"]
