...
class NextDiT(nn.Module):
    ...
    def __init__(
        ...
        # ====== 新增：结构先验编码器 (Mask Embedder) ======
        # 功能：将单通道掩码映射到与图像特征相同的维度 (dim)
        # 默认参数：in_features 为 patch_size*patch_size*1 (单通道)，init_method 为 nn.init.zeros_
        # 零初始化的作用：在训练初期，结构先验的输出全为 0，相当于原版 Text-to-Image，避免破坏预训练权重。
        self.mask_embedder = ColumnParallelLinear(
            in_features=patch_size * patch_size * 1,
            out_features=dim,
            bias=True,
            gather_output=True,
            init_method=nn.init.zeros_,
        )
        if hasattr(self.mask_embedder, "bias") and self.mask_embedder.bias is not None:
            nn.init.constant_(self.mask_embedder.bias, 0.0)
        # ===================================================
    ...
    def patchify_and_embed(
        ...
        if isinstance(x, torch.Tensor):
            ...
            if struct_mask is not None:
                # struct_mask 形状应为 [B, 1, H, W]
                sm = struct_mask.view(B, 1, H // pH, pH, W // pW, pW).permute(0, 2, 4, 1, 3, 5).flatten(3)
                sm_embed = self.mask_embedder(sm)
                sm_embed = sm_embed.flatten(1, 2)
                x = x + sm_embed # 特征空间融合，将结构信息注入图像潜变量
            # ==============================
