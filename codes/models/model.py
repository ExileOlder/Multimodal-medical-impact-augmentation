    ...
    def forward(
        ...
        if dtype in [torch.float16, torch.bfloat16]:
            ...
            if self.n_local_heads != self.n_local_kv_heads:
                xk_attn = xk.unsqueeze(3).repeat(1, 1, 1, self.n_rep, 1).flatten(2, 3)
                xv_attn = xv.unsqueeze(3).repeat(1, 1, 1, self.n_rep, 1).flatten(2, 3)
            else:
                xk_attn, xv_attn = xk, xv

            output = (
                F.scaled_dot_product_attention(
                    xq.permute(0, 2, 1, 3),
                    xk_attn.permute(0, 2, 1, 3),
                    xv_attn.permute(0, 2, 1, 3),
